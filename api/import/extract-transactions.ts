...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const receiptModeHint = targetType === "income"
    ...
    : "Extract EVERY individual line item as its own separate row — do NOT pre-aggregate or merge any items. Each row gets its own date, merchant (store name, same for all items on same receipt), amount (that line item's price), category (closest match), and notes (item name/description). Distribute tax proportionally across items, or add a separate row labeled 'Tax'. The goal is maximum granularity at extraction time. If a row is a discount, coupon, instant savings, or member savings that applies to the immediately preceding line item (identified by negative amount, 'SAVINGS', 'COUPON', 'REBATE', article/SKU reference, or similar label), do NOT emit it as a separate row. Instead subtract its absolute value from the preceding item's amount (result must be >= 0, floor at 0 if discount exceeds price) and append to that item's notes: ' (disc: -X.XX)' where X.XX is the discount amount. Only emit a row as standalone if it is a general order-level discount not tied to any specific item.";
  ...
  for (const file of uploadedFiles) {
    try {
      ...
      const prompt = [
        ...
        "- pre_tax_amount (number, optional, amount before tax for this row)",
        "- tax_amount (number, optional, tax assigned to this row)",
        "- taxable (boolean, optional, true if this row is taxable)",
        "- receipt_tax_total (number, optional, total tax for entire receipt/document)",
        "- receipt_subtotal (number, optional, subtotal before tax for entire receipt/document)",
      ...
      for (const item of parsed) {
        candidates.push({
          ...
          pre_tax_amount: typeof item.pre_tax_amount === "number" ? item.pre_tax_amount : Number(item.pre_tax_amount) || undefined,
          tax_amount: typeof item.tax_amount === "number" ? item.tax_amount : Number(item.tax_amount) || undefined,
          taxable: typeof item.taxable === "boolean" ? item.taxable : undefined,
          receipt_tax_total: typeof item.receipt_tax_total === "number" ? item.receipt_tax_total : Number(item.receipt_tax_total) || undefined,
          receipt_subtotal: typeof item.receipt_subtotal === "number" ? item.receipt_subtotal : Number(item.receipt_subtotal) || undefined,
